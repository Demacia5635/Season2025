 ...
 * DemaciaPoseEstimator#addVisionMeasurement} and only call {@link DemaciaPoseEstimator#update} then this will
 * behave exactly the same as Odometry.
 *
 * <p>{@link DemaciaPoseEstimator#update} should be called every robot loop.
 *
 * <p>{@link DemaciaPoseEstimator#addVisionMeasurement} can be called as infrequently as you want; if you
 * never call it then this class will behave exactly like regular encoder odometry.
 *
 * @param <T> Wheel positions type.
 */
public class DemaciaPoseEstimator<T> {
  ...
  public Pose2d calcEstimatedPose(Rotation2d currentGyroAngle, SwerveModulePosition[] currentWheelPositions){
    return m_odometry.calcEstimatedPose(currentGyroAngle, currentWheelPositions);
  }
   ...
   * DemaciaPoseEstimator#update} every loop.
   *
   ...
   *     DemaciaPoseEstimator#updateWithTime(double,Rotation2d,WheelPositions)} then you must use a
   ...
   * DemaciaPoseEstimator#setVisionMeasurementStdDevs(Matrix)} or this method.
   *
   * @param visionRobotPoseMeters The pose of the robot as measured by the vision camera.
   * @param timestampSeconds The timestamp of the vision measurement in seconds. Note that if you
  ...
  private class InterpolationRecord implements Interpolatable<InterpolationRecord> {
    ...
    public InterpolationRecord interpolate(InterpolationRecord endValue, double t) {
      if (t < 0) {
        ...
        Twist2d twist = m_kinematics.toTwist2d(wheelPositions, (SwerveModulePosition[]) wheelLerp);
        twist.dtheta = gyroLerp.minus(gyroAngle).getRadians();

        return new InterpolationRecord(poseMeters.exp(twist), (Rotation2d)gyroLerp, wheelLerp);
      }
    }
