...
public class Auto_3Coral extends Command {
  ...
  public void initialize() {
    cmd = goToMultiple(new PathPoint[]{dumyPoint, scoring1} ,2.0, Rotation2d.fromDegrees(50));
    cmd = cmd.andThen(new WaitCommand(2));
    cmd = cmd.andThen(goToMultiple(new PathPoint[]{dummyPoint, feeder}, 1.5, feeder.getRotation()));
    cmd = cmd.andThen(new WaitCommand(2));
    cmd = cmd.andThen(goToMultiple(new PathPoint[]{dummyPoint, new PathPoint(new Translation2d(16, 1.24), Rotation2d.fromDegrees(0))}, 2, Rotation2d.fromDegrees(-50)));
    cmd = cmd.andThen(new WaitCommand(0.5));
    cmd = cmd.andThen(goToMultiple(new PathPoint[]{dummyPoint, scoring1}, 2.5, Rotation2d.fromDegrees(50)));
    cmd = cmd.andThen(new WaitCommand(2));
