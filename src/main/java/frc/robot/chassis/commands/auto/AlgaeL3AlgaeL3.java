...
public class AlgaeL3AlgaeL3 extends SequentialCommandGroup {
  ...
  public AlgaeL3AlgaeL3(Chassis chassis, Arm arm, Gripper gripper, boolean isRed, boolean isRight) {
    ...
    FieldTarget algaePointFirst = new FieldTarget(isRight ? POSITION.D : POSITION.F, ELEMENT_POSITION.ALGEA,
        LEVEL.ALGAE_TOP);
    FieldTarget algaePointSecond = new FieldTarget(isRight ? POSITION.C : POSITION.A, ELEMENT_POSITION.ALGEA,
        LEVEL.ALGAE_BOTTOM);
    ...
    addCommands(
        ...
        new FollowTrajectory(chassis, secondCoral).withTimeout(2),
        new WaitCommand(0.1).alongWith(new InstantCommand(() -> new AlignCoral(gripper).schedule())),
        (new WaitUntilCommand(() -> !gripper.isCoralDownSensor())
            .alongWith(new InstantCommand(() -> new Drop(gripper).schedule())))
            .withTimeout(0.7),
     ...
     * {
     * add(dummyPoint);
     * add(feeder.getApproachingPoint());
     * }
     * }, correctRotation(-130))
     ...
     * new WaitUntilCommand(() -> gripper.isCoralDownSensor()),
     * new RunCommand(() -> chassis.setRobotRelVelocities(new ChassisSpeeds(-2, 0,
     * 0)),chassis).withTimeout(0.1),
     * (new FollowTrajectory(chassis, new ArrayList<PathPoint>() {
     * {
     * add(dummyPoint);
     * add(infrontReef);
     ...
     * !(gripper.getCurrentCommand() instanceof Drop) ? new InstantCommand(()-> new
     * Drop(gripper).schedule()) : new InstantCommand(),
     * new WaitUntilCommand(() -> !gripper.isCoralUpSensor()).alongWith(new
     * InstantCommand(() -> new Drop(gripper).schedule())),
     * new WaitCommand(0.1),
     * new RunCommand(()-> chassis.setRobotRelVelocities(new ChassisSpeeds(-2, 0,
     * 0)), chassis).withTimeout(0.2),
     * (new FollowTrajectory(chassis, new ArrayList<PathPoint>() {
     * {
     * add(dummyPoint);
     * add(feeder.getApproachingPoint());
